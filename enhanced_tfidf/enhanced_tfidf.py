...
def get_summary(filename, title, language):
    ...
    for i in range(length):
        ...
        score = txt_data[i]['num_words'] - txt_data[i]['url_email'] + txt_data[i]['special_chars'] + txt_data[i]['numbers'] + \
            txt_data[i]['quote_chars'] + txt_data[i]['title_words'] + \
            txt_data[i]['nouns'] + txt_data[i]['position'] + \
            txt_data[i]['tf-idf']
        d['score'] = score
        all_scores += score
...
filenm = './enhanced_tfidf/marathi_pos.pickle'
